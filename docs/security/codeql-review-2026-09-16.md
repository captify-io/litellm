...
Upload previews accept browser blob URLs. Stored plugin URLs remain visible as text, but only HTTP or HTTPS URLs without embedded credentials become links or pass form validation. Regression tests cover active-content URL schemes, embedded credentials and valid blob previews. The existing onboarding component already requires a server-supplied invitation ID and has no Math.random call; alert 7's current location does not match its reported expression. Keep that alert pending fresh provider analysis rather than inventing a code change or dismissing it
