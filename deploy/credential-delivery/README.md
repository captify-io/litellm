...
The existing `AWS_ROLE_NAME` setting is shared with other AWS integrations. Review every consumer before changing it. This source correction does not enable IAM database authentication, provision a database user or change existing runtime credentials. Native database access, renewal and application acceptance remain separate qualification steps
